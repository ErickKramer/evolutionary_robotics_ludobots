...
class INDIVIDUAL:
    def __init__(self, id):
        ...
        self.genome = np.random.random((4,8))*2 - 1.0
        self.fitness = 0
        self.id = id
    ...
    def compute_fitness(self):
        ...
        positionData_y = self.sim.get_sensor_data(sensor_id=self.robot.position_body, svi=1)
    ...
    def mutate(self):
        ...
        gene_to_mutate_i = np.random.randint(0, 4) # Randomly choose the gene to mutate
        gene_to_mutate_j = np.random.randint(0, 8)  # Randomly choose the gene to mutate
        self.genome[gene_to_mutate_i, gene_to_mutate_j] = random.gauss(self.genome[gene_to_mutate_i, gene_to_mutate_j],
                                                   math.fabs(self.genome[gene_to_mutate_i, gene_to_mutate_j]))

        # clip value between [-1,1]
        if self.genome[gene_to_mutate_i, gene_to_mutate_j] > 1:
            self.genome[gene_to_mutate_i, gene_to_mutate_j] = 1

        elif self.genome[gene_to_mutate_i, gene_to_mutate_j] < -1:
            self.genome[gene_to_mutate_i, gene_to_mutate_j] = -1
